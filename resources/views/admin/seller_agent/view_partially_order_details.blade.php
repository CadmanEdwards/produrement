...
continue;
  ...
  if(is_null($item->received_quantity)){
    $quantity_test = $item->quantity;
  }else{
    $quantity_test = $item->received_quantity;
  }
$image_data = explode(',',$item->images);
$total_price = $quantity_test*$item->price;
$total_sum_price += $quantity_test*$item->price;
$total_actual_sum_price += $quantity_test*$item->acutal_price;
